...
-spec set_header(request(AQS), binary(), binary()) -> request(AQS).
...
-spec prepend_header(request(AQV), binary(), binary()) -> request(AQV).
...
-spec set_body(request(any()), ARA) -> request(ARA).
set_body(Req, Body) ->
...
-spec map(request(ARC), fun((ARC) -> ARE)) -> request(ARE).
...
-spec set_query(request(ARO), list({binary(), binary()})) -> request(ARO).
set_query(Req, Query) ->
    Pair = fun(T) ->
...
-spec set_method(request(ARS), gleam@http:method()) -> request(ARS).
...
-spec set_scheme(request(ARZ), gleam@http:scheme()) -> request(ARZ).
...
-spec set_host(request(ASC), binary()) -> request(ASC).
set_host(Req, Host) ->
    _record = Req,
    {request,
        erlang:element(2, _record),
...
-spec set_port(request(ASF), integer()) -> request(ASF).
set_port(Req, Port) ->
    _record = Req,
    {request,
        erlang:element(2, _record),
...
-spec set_path(request(ASI), binary()) -> request(ASI).
set_path(Req, Path) ->
    _record = Req,
    {request,
        erlang:element(2, _record),
...
-spec set_cookie(request(ASL), binary(), binary()) -> request(ASL).
...
-spec remove_cookie(request(ASQ), binary()) -> request(ASQ).
