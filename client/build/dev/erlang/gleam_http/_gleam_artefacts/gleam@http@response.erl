...
-type response(BAT) :: {response, integer(), list({binary(), binary()}), BAT}.
...
-spec set_header(response(BBI), binary(), binary()) -> response(BBI).
...
-spec prepend_header(response(BBL), binary(), binary()) -> response(BBL).
...
-spec set_body(response(any()), BBQ) -> response(BBQ).
...
-spec try_map(response(BAU), fun((BAU) -> {ok, BAW} | {error, BAX})) -> {ok,
        response(BAW)} |
    {error, BAX}.
...
-spec map(response(BBS), fun((BBS) -> BBU)) -> response(BBU).
...
-spec expire_cookie(response(BCC), binary(), gleam@http@cookie:attributes()) -> response(BCC).
