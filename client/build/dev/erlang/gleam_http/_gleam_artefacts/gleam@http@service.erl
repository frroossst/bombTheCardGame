...
-spec map_response_body(
    fun((BFI) -> gleam@http@response:response(BEU)),
    fun((BEU) -> BEV)
) -> fun((BFI) -> gleam@http@response:response(BEV)).
...
-spec prepend_response_header(
    fun((BFM) -> gleam@http@response:response(BFO)),
    binary(),
    binary()
) -> fun((BFM) -> gleam@http@response:response(BFO)).
...
-spec ensure_post(gleam@http@request:request(BEZ)) -> {ok,
        gleam@http@request:request(BEZ)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(BGV)) -> BHD)) -> fun((gleam@http@request:request(BGV)) -> BHD).
