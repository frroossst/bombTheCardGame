...
-spec do_decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, FLJ} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, FLJ} | {error, decode_error()}.
...
-spec decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, FKX} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, FKX} | {error, decode_error()}.
decode(Json, Decoder) ->
    do_decode(Json, Decoder).
...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(FLT)) -> {ok, FLT} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(FLF)) -> {ok, FLF} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(FLB)) -> {ok, FLB} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(FLZ), fun((FLZ) -> json())) -> json().
...
-spec array(list(FMD), fun((FMD) -> json())) -> json().
