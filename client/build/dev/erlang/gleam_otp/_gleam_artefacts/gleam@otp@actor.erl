...
-type message(FPA) :: {message, FPA} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-type next(FPB, FPC) :: {continue,
        FPC,
        gleam@option:option(gleam@erlang@process:selector(FPB))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type init_result(FPD, FPE) :: {ready, FPD, gleam@erlang@process:selector(FPE)} |
    {failed, binary()}.

-type self(FPF, FPG) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        FPF,
        gleam@erlang@process:subject(FPG),
        gleam@erlang@process:selector(message(FPG)),
        gleam@otp@system:debug_state(),
        fun((FPG, FPF) -> next(FPG, FPF))}.

-type spec(FPH, FPI) :: {spec,
        fun(() -> init_result(FPH, FPI)),
        integer(),
...
-spec with_selector(next(FPU, FPV), gleam@erlang@process:selector(FPU)) -> next(FPU, FPV).
...
-spec selecting_system_messages(gleam@erlang@process:selector(message(FQG))) -> gleam@erlang@process:selector(message(FQG)).
...
-spec receive_message(self(any(), FQC)) -> message(FQC).
...
-spec init_selector(
    gleam@erlang@process:subject(FVG),
    gleam@erlang@process:selector(FVG)
) -> gleam@erlang@process:selector(message(FVG)).
...
-spec initialise_actor(
    spec(any(), FQX),
    gleam@erlang@process:subject({ok, gleam@erlang@process:subject(FQX)} |
        {error, gleam@erlang@process:exit_reason()})
...
-spec start_spec(spec(any(), FRK)) -> {ok, gleam@erlang@process:subject(FRK)} |
    {error, start_error()}.
start_spec(Spec) ->
...
-spec start(FRQ, fun((FRR, FRQ) -> next(FRR, FRQ))) -> {ok,
        gleam@erlang@process:subject(FRR)} |
    {error, start_error()}.
start(State, Loop) ->
    start_spec(
        {spec,
...
-spec send(gleam@erlang@process:subject(FRX), FRX) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(FRZ),
    fun((gleam@erlang@process:subject(FSB)) -> FRZ),
    integer()
) -> FSB.
