...
-type spec(GFH, GFI) :: {spec,
        GFH,
        integer(),
        integer(),
        fun((children(GFH)) -> children(GFI))}.

-opaque children(GFJ) :: {ready, starter(GFJ)} | {failed, child_start_error()}.

-opaque child_spec(GFK, GFL, GFM) :: {child_spec,
        fun((GFL) -> {ok, gleam@erlang@process:subject(GFK)} |
            {error, gleam@otp@actor:start_error()}),
        fun((GFL, gleam@erlang@process:subject(GFK)) -> GFM)}.
...
-spec start_child(child_spec(any(), GFT, GFU), GFT) -> {ok, child(GFU)} |
    {error, child_start_error()}.
...
-spec perform_instruction_for_child(
    GGH,
    instruction(),
    child_spec(any(), GGH, GGJ),
    child(GGJ)
) -> {ok, {child(GGJ), instruction()}} | {error, child_start_error()}.
...
-spec start_and_add_child(starter(GHA), child_spec(any(), GHA, GHD)) -> children(GHD).
...
-spec add(children(GHI), child_spec(any(), GHI, GHL)) -> children(GHL).
...
-spec supervisor(
    fun((GHQ) -> {ok, gleam@erlang@process:subject(GHR)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(GHR, GHQ, GHQ).
supervisor(Start) ->
...
-spec worker(
    fun((GHY) -> {ok, gleam@erlang@process:subject(GHZ)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(GHZ, GHY, GHY).
worker(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.
...
-spec returning(
    child_spec(GIG, GIH, any()),
    fun((GIH, gleam@erlang@process:subject(GIG)) -> GIN)
) -> child_spec(GIG, GIH, GIN).
returning(Child, Updater) ->
    {child_spec, erlang:element(2, Child), Updater}.

-file("src/gleam/otp/supervisor.gleam", 248).
-spec init(spec(any(), GIS)) -> gleam@otp@actor:init_result(state(GIS), message()).
init(Spec) ->
...
-spec handle_exit(gleam@erlang@process:pid_(), state(GIY)) -> gleam@otp@actor:next(message(), state(GIY)).
...
-spec loop(message(), state(GJD)) -> gleam@otp@actor:next(message(), state(GJD)).
loop(Message, State) ->
    case Message of
        {exit, Exit_message} ->
