...
-opaque task(GSQ) :: {task,
        gleam@erlang@process:pid_(),
        gleam@erlang@process:pid_(),
        gleam@erlang@process:subject(GSQ)}.

-type await_error() :: timeout | {exit, gleam@dynamic:dynamic_()}.

-type message2(GSR, GSS) :: {m2_from_subject1, GSR} |
    {m2_from_subject2, GSS} |
    m2_timeout.

-type message3(GST, GSU, GSV) :: {m3_from_subject1, GST} |
    {m3_from_subject2, GSU} |
    {m3_from_subject3, GSV} |
    m3_timeout.

-type message4(GSW, GSX, GSY, GSZ) :: {m4_from_subject1, GSW} |
    {m4_from_subject2, GSX} |
    {m4_from_subject3, GSY} |
    {m4_from_subject4, GSZ} |
    m4_timeout.

-type message(GTA) :: {message, integer(), GTA} | message_timeout.
...
-spec try_await(task(GTF), integer()) -> {ok, GTF} | {error, await_error()}.
...
-spec await(task(GTJ), integer()) -> GTJ.
await(Task, Timeout) ->
...
-spec await_forever(task(GTN)) -> GTN.
await_forever(Task) ->
    assert_owner(Task),
    Selector = begin
        _pipe = gleam_erlang_ffi:new_selector(),
...
-spec try_await2_loop(
    gleam@erlang@process:selector(message2(GTX, GTY)),
    gleam@option:option({ok, GTX} | {error, await_error()}),
    gleam@option:option({ok, GTY} | {error, await_error()}),
    gleam@erlang@process:timer()
) -> {{ok, GTX} | {error, await_error()}, {ok, GTY} | {error, await_error()}}.
...
-spec try_await2(task(GTP), task(GTR), integer()) -> {{ok, GTP} |
        {error, await_error()},
    {ok, GTR} | {error, await_error()}}.
...
-spec try_await3_loop(
    gleam@erlang@process:selector(message3(GUY, GUZ, GVA)),
    gleam@option:option({ok, GUY} | {error, await_error()}),
    gleam@option:option({ok, GUZ} | {error, await_error()}),
    gleam@option:option({ok, GVA} | {error, await_error()}),
    gleam@erlang@process:timer()
) -> {{ok, GUY} | {error, await_error()},
    {ok, GUZ} | {error, await_error()},
...
-spec try_await3(task(GUM), task(GUO), task(GUQ), integer()) -> {{ok, GUM} |
        {error, await_error()},
    {ok, GUO} | {error, await_error()},
...
-spec try_await4_loop(
    gleam@erlang@process:selector(message4(GWK, GWL, GWM, GWN)),
    gleam@option:option({ok, GWK} | {error, await_error()}),
    gleam@option:option({ok, GWL} | {error, await_error()}),
    gleam@option:option({ok, GWM} | {error, await_error()}),
    gleam@option:option({ok, GWN} | {error, await_error()}),
    gleam@erlang@process:timer()
) -> {{ok, GWK} | {error, await_error()},
    {ok, GWL} | {error, await_error()},
...
-spec try_await4(task(GVU), task(GVW), task(GVY), task(GWA), integer()) -> {{ok,
            GVU} |
        {error, await_error()},
    {ok, GVW} | {error, await_error()},
...
-spec dict_to_list(gleam@dict:dict(integer(), GYD), integer(), GYD) -> list(GYD).
...
-spec try_await_all_loop(
    gleam@dict:dict(integer(), {ok, GXT} | {error, await_error()}),
    integer(),
    gleam@erlang@process:timer(),
    gleam@erlang@process:selector(message(GXT))
) -> list({ok, GXT} | {error, await_error()}).
...
-spec try_await_all(list(task(GXN)), integer()) -> list({ok, GXN} |
    {error, await_error()}).
