...
-opaque app(KIL, KIM, KIN) :: {app,
        fun((KIL) -> {KIM, lustre@effect:effect(KIN)}),
        fun((KIM, KIN) -> {KIM, lustre@effect:effect(KIN)}),
        fun((KIM) -> lustre@internals@vdom:element(KIN)),
...
-spec application(
    fun((KJG) -> {KJH, lustre@effect:effect(KJI)}),
    fun((KJH, KJI) -> {KJH, lustre@effect:effect(KJI)}),
    fun((KJH) -> lustre@internals@vdom:element(KJI))
) -> app(KJG, KJH, KJI).
...
-spec element(lustre@internals@vdom:element(KIU)) -> app(nil, nil, KIU).
element(Html) ->
...
-spec simple(
    fun((KIZ) -> KJA),
    fun((KJA, KJB) -> KJA),
    fun((KJA) -> lustre@internals@vdom:element(KJB))
) -> app(KIZ, KJA, KJB).
...
-spec component(
    fun((KJP) -> {KJQ, lustre@effect:effect(KJR)}),
    fun((KJQ, KJR) -> {KJQ, lustre@effect:effect(KJR)}),
    fun((KJQ) -> lustre@internals@vdom:element(KJR)),
    gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, KJR} |
        {error, list(gleam@dynamic:decode_error())}))
) -> app(KJP, KJQ, KJR).
...
-spec do_start(app(KKL, any(), KKN), binary(), KKL) -> {ok,
        fun((lustre@internals@runtime:action(KKN, client_spa())) -> nil)} |
    {error, error()}.
do_start(_, _, _) ->
    {error, not_a_browser}.

-file("src/lustre.gleam", 444).
-spec do_start_actor(app(KLQ, any(), KLS), KLQ) -> {ok,
        gleam@erlang@process:subject(lustre@internals@runtime:action(KLS, server_component()))} |
    {error, error()}.
...
-spec start_actor(app(KLF, any(), KLH), KLF) -> {ok,
        gleam@erlang@process:subject(lustre@internals@runtime:action(KLH, server_component()))} |
    {error, error()}.
...
-spec start_server_component(app(KKV, any(), KKX), KKV) -> {ok,
        fun((lustre@internals@runtime:action(KKX, server_component())) -> nil)} |
    {error, error()}.
...
-spec dispatch(KMI) -> lustre@internals@runtime:action(KMI, any()).
dispatch(Msg) ->
    {dispatch, Msg}.
...
-spec start(app(KKB, any(), KKD), binary(), KKB) -> {ok,
        fun((lustre@internals@runtime:action(KKD, client_spa())) -> nil)} |
    {error, error()}.
