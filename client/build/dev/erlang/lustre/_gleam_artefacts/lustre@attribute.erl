...
-spec on(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, IEL} |
        {error, list(gleam@dynamic:decode_error())})
) -> lustre@internals@vdom:attribute(IEL).
on(Name, Handler) ->
...
-spec map(lustre@internals@vdom:attribute(IEQ), fun((IEQ) -> IES)) -> lustre@internals@vdom:attribute(IES).
map(Attr, F) ->
    case Attr of
