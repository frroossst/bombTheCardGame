...
-opaque effect(HNP) :: {effect, list(fun((actions(HNP)) -> nil))}.

-type actions(HNQ) :: {actions,
        fun((HNQ) -> nil),
        fun((binary(), gleam@json:json()) -> nil),
        fun((gleam@erlang@process:selector(HNQ)) -> nil),
        gleam@dynamic:dynamic_()}.

-file("src/lustre/effect.gleam", 126).
?DOC(false).
-spec custom(
    fun((fun((HNV) -> nil), fun((binary(), gleam@json:json()) -> nil), fun((gleam@erlang@process:selector(HNV)) -> nil), gleam@dynamic:dynamic_()) -> nil)
) -> effect(HNV).
custom(Run) ->
    {effect,
        [fun(Actions) ->
...
-spec from(fun((fun((HNR) -> nil)) -> nil)) -> effect(HNR).
from(Effect) ->
...
-spec batch(list(effect(HOA))) -> effect(HOA).
batch(Effects) ->
    {effect,
        begin
            gleam@list:fold(
...
-spec map(effect(HOE), fun((HOE) -> HOG)) -> effect(HOG).
map(Effect, F) ->
    {effect,
        begin
            gleam@list:map(
...
-spec perform(
    ...
    fun((HOI) -> nil),
    fun((binary(), gleam@json:json()) -> nil),
    fun((gleam@erlang@process:selector(HOI)) -> nil),
    gleam@dynamic:dynamic_()
) -> nil.
