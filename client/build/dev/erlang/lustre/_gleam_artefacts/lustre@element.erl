...
-spec element(
    binary(),
    list(lustre@internals@vdom:attribute(IMU)),
    list(lustre@internals@vdom:element(IMU))
) -> lustre@internals@vdom:element(IMU).
...
-spec do_keyed(lustre@internals@vdom:element(INH), binary()) -> lustre@internals@vdom:element(INH).
do_keyed(El, Key) ->
    case El of
...
-spec keyed(
    fun((list(lustre@internals@vdom:element(INA))) -> lustre@internals@vdom:element(INA)),
    list({binary(), lustre@internals@vdom:element(INA)})
) -> lustre@internals@vdom:element(INA).
keyed(El, Children) ->
    El(
        begin
            gleam@list:map(
...
-spec namespaced(
    binary(),
    binary(),
    list(lustre@internals@vdom:attribute(INK)),
    list(lustre@internals@vdom:element(INK))
) -> lustre@internals@vdom:element(INK).
...
-spec advanced(
    binary(),
    binary(),
    list(lustre@internals@vdom:attribute(INQ)),
    list(lustre@internals@vdom:element(INQ)),
    boolean(),
    boolean()
) -> lustre@internals@vdom:element(INQ).
...
-spec fragment(list(lustre@internals@vdom:element(IOA))) -> lustre@internals@vdom:element(IOA).
fragment(Elements) ->
    element(
...
-spec map(lustre@internals@vdom:element(IOE), fun((IOE) -> IOG)) -> lustre@internals@vdom:element(IOG).
map(Element, F) ->
    case Element of
        {text, Content} ->
            {text, Content};
...
?DOC(false).
-spec get_root(fun((fun((IOI) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> lustre@effect:effect(IOI).
get_root(Effect) ->
