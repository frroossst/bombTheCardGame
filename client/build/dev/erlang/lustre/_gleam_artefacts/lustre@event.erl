...
-spec on(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, MLO} |
        {error, list(gleam@dynamic:decode_error())})
) -> lustre@internals@vdom:attribute(MLO).
on(Name, Handler) ->
    lustre@attribute:on(Name, Handler).

-file("src/lustre/event.gleam", 43).
?DOC("\n").
-spec on_click(MLR) -> lustre@internals@vdom:attribute(MLR).
on_click(Msg) ->
    on(<<"click"/utf8>>, fun(_) -> {ok, Msg} end).

-file("src/lustre/event.gleam", 49).
?DOC("\n").
-spec on_mouse_down(MLT) -> lustre@internals@vdom:attribute(MLT).
on_mouse_down(Msg) ->
    on(<<"mousedown"/utf8>>, fun(_) -> {ok, Msg} end).

-file("src/lustre/event.gleam", 55).
?DOC("\n").
-spec on_mouse_up(MLV) -> lustre@internals@vdom:attribute(MLV).
on_mouse_up(Msg) ->
    on(<<"mouseup"/utf8>>, fun(_) -> {ok, Msg} end).

-file("src/lustre/event.gleam", 61).
?DOC("\n").
-spec on_mouse_enter(MLX) -> lustre@internals@vdom:attribute(MLX).
on_mouse_enter(Msg) ->
    on(<<"mouseenter"/utf8>>, fun(_) -> {ok, Msg} end).

-file("src/lustre/event.gleam", 67).
?DOC("\n").
-spec on_mouse_leave(MLZ) -> lustre@internals@vdom:attribute(MLZ).
on_mouse_leave(Msg) ->
    on(<<"mouseleave"/utf8>>, fun(_) -> {ok, Msg} end).

-file("src/lustre/event.gleam", 73).
?DOC("\n").
-spec on_mouse_over(MMB) -> lustre@internals@vdom:attribute(MMB).
on_mouse_over(Msg) ->
    on(<<"mouseover"/utf8>>, fun(_) -> {ok, Msg} end).

-file("src/lustre/event.gleam", 79).
?DOC("\n").
-spec on_mouse_out(MMD) -> lustre@internals@vdom:attribute(MMD).
on_mouse_out(Msg) ->
...
-spec on_keypress(fun((binary()) -> MMF)) -> lustre@internals@vdom:attribute(MMF).
on_keypress(Msg) ->
...
-spec on_keydown(fun((binary()) -> MMH)) -> lustre@internals@vdom:attribute(MMH).
on_keydown(Msg) ->
...
-spec on_keyup(fun((binary()) -> MMJ)) -> lustre@internals@vdom:attribute(MMJ).
on_keyup(Msg) ->
...
-spec on_focus(MMR) -> lustre@internals@vdom:attribute(MMR).
on_focus(Msg) ->
    on(<<"focus"/utf8>>, fun(_) -> {ok, Msg} end).

-file("src/lustre/event.gleam", 154).
-spec on_blur(MMT) -> lustre@internals@vdom:attribute(MMT).
on_blur(Msg) ->
...
?DOC("\n").
-spec on_input(fun((binary()) -> MML)) -> lustre@internals@vdom:attribute(MML).
on_input(Msg) ->
...
-spec on_check(fun((boolean()) -> MMN)) -> lustre@internals@vdom:attribute(MMN).
on_check(Msg) ->
...
-spec on_submit(MMP) -> lustre@internals@vdom:attribute(MMP).
on_submit(Msg) ->
    on(
        <<"submit"/utf8>>,
        fun(Event) ->
