...
-type patch(IRW) :: {diff, element_diff(IRW)} |
    {emit, binary(), gleam@json:json()} |
    {init, list(binary()), lustre@internals@vdom:element(IRW)}.

-type element_diff(IRX) :: {element_diff,
        gleam@dict:dict(binary(), lustre@internals@vdom:element(IRX)),
        gleam@set:set(binary()),
        gleam@dict:dict(binary(), attribute_diff(IRX)),
        gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, IRX} |
            {error, list(gleam@dynamic:decode_error())}))}.

-type attribute_diff(IRY) :: {attribute_diff,
        gleam@set:set(lustre@internals@vdom:attribute(IRY)),
        gleam@set:set(binary()),
        gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, IRY} |
            {error, list(gleam@dynamic:decode_error())}))}.
...
-spec do_attribute(
    attribute_diff(ISX),
    binary(),
    {ok, lustre@internals@vdom:attribute(ISX)} | {error, nil},
    {ok, lustre@internals@vdom:attribute(ISX)} | {error, nil}
) -> attribute_diff(ISX).
...
?DOC(false).
-spec zip(list(ITO), list(ITO)) -> list({gleam@option:option(ITO),
    gleam@option:option(ITO)}).
zip(Xs, Ys) ->
    case {Xs, Ys} of
        {[], []} ->
            [];
...
?DOC(false).
-spec event_handler(lustre@internals@vdom:attribute(IUA)) -> {ok,
        {binary(),
            fun((gleam@dynamic:dynamic_()) -> {ok, IUA} |
                {error, list(gleam@dynamic:decode_error())})}} |
    {error, nil}.
...
?DOC(false).
-spec attribute_dict(list(lustre@internals@vdom:attribute(ITU))) -> gleam@dict:dict(binary(), lustre@internals@vdom:attribute(ITU)).
...
?DOC(false).
-spec attributes(
    list(lustre@internals@vdom:attribute(ISR)),
    list(lustre@internals@vdom:attribute(ISR))
) -> attribute_diff(ISR).
attributes(Old, New) ->
...
-spec do_element_list(
    element_diff(ISK),
    list(lustre@internals@vdom:element(ISK)),
    list(lustre@internals@vdom:element(ISK)),
    binary()
) -> element_diff(ISK).
...
-spec do_elements(
    element_diff(ISD),
    gleam@option:option(lustre@internals@vdom:element(ISD)),
    gleam@option:option(lustre@internals@vdom:element(ISD)),
    binary()
) -> element_diff(ISD).
...
?DOC(false).
-spec elements(
    lustre@internals@vdom:element(IRZ),
    lustre@internals@vdom:element(IRZ)
) -> element_diff(IRZ).
elements(Old, New) ->
    do_elements(
...
-spec fold_element_list_event_handlers(
    gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, IUN} |
        {error, list(gleam@dynamic:decode_error())})),
    list(lustre@internals@vdom:element(IUN)),
    binary()
) -> gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, IUN} |
    {error, list(gleam@dynamic:decode_error())})).
...
-spec fold_event_handlers(
    gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, IUF} |
        {error, list(gleam@dynamic:decode_error())})),
    lustre@internals@vdom:element(IUF),
    binary()
) -> gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, IUF} |
    {error, list(gleam@dynamic:decode_error())})).
