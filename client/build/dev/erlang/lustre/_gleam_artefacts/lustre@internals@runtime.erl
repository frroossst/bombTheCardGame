...
-type state(JME, JMF, JMG) :: {state,
        gleam@erlang@process:subject(action(JMF, JMG)),
        gleam@erlang@process:selector(action(JMF, JMG)),
        JME,
        fun((JME, JMF) -> {JME, lustre@effect:effect(JMF)}),
        fun((JME) -> lustre@internals@vdom:element(JMF)),
        lustre@internals@vdom:element(JMF),
        gleam@dict:dict(binary(), fun((lustre@internals@patch:patch(JMF)) -> nil)),
        gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, JMF} |
            {error, list(gleam@dynamic:decode_error())})),
        gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, JMF} |
            {error, list(gleam@dynamic:decode_error())}))}.

-type action(JMH, JMI) :: {attrs, list({binary(), gleam@dynamic:dynamic_()})} |
    {batch, list(JMH), lustre@effect:effect(JMH)} |
    {debug, debug_action()} |
    {dispatch, JMH} |
    {emit, binary(), gleam@json:json()} |
    {event, binary(), gleam@dynamic:dynamic_()} |
    {set_selector, gleam@erlang@process:selector(JMH)} |
    shutdown |
    {subscribe, binary(), fun((lustre@internals@patch:patch(JMH)) -> nil)} |
    {unsubscribe, binary()} |
    {gleam_phantom, JMI}.
...
?DOC(false).
-spec run_renderers(
    gleam@dict:dict(any(), fun((lustre@internals@patch:patch(JNN)) -> nil)),
    lustre@internals@patch:patch(JNN)
) -> nil.
...
?DOC(false).
-spec run_effects(
    lustre@effect:effect(JNS),
    gleam@erlang@process:subject(action(JNS, any()))
) -> nil.
...
?DOC(false).
-spec loop(action(JMX, JMY), state(JNB, JMX, JMY)) -> gleam@otp@actor:next(action(JMX, JMY), state(JNB, JMX, JMY)).
loop(Message, State) ->
    case Message of
        {attrs, Attrs} ->
...
?DOC(false).
-spec start(
    {JMJ, lustre@effect:effect(JMK)},
    fun((JMJ, JMK) -> {JMJ, lustre@effect:effect(JMK)}),
    fun((JMJ) -> lustre@internals@vdom:element(JMK)),
    gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, JMK} |
        {error, list(gleam@dynamic:decode_error())}))
) -> {ok, gleam@erlang@process:subject(action(JMK, any()))} |
