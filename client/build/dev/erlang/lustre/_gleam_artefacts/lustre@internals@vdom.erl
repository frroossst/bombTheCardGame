...
-type attribute(HSE) :: {attribute,
        binary(),
        gleam@dynamic:dynamic_(),
        boolean()} |
    {event,
        binary(),
        fun((gleam@dynamic:dynamic_()) -> {ok, HSE} |
            {error, list(gleam@dynamic:decode_error())})}.

-file("src/lustre/internals/vdom.gleam", 508).
?DOC(false).
-spec attribute_to_event_handler(attribute(HUG)) -> {ok,
        {binary(),
...
-spec do_element_list_handlers(
    list(element(HSS)),
    gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, HSS} |
        {error, list(gleam@dynamic:decode_error())})),
    binary()
) -> gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, HSS} |
    {error, list(gleam@dynamic:decode_error())})).
...
-spec do_handlers(
    element(HSK),
    gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, HSK} |
        {error, list(gleam@dynamic:decode_error())})),
    binary()
) -> gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, HSK} |
    {error, list(gleam@dynamic:decode_error())})).
...
?DOC(false).
-spec handlers(element(HSF)) -> gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok,
        HSF} |
    {error, list(gleam@dynamic:decode_error())})).
handlers(Element) ->
