...
-spec component(list(lustre@internals@vdom:attribute(MQQ))) -> lustre@internals@vdom:element(MQQ).
component(Attrs) ->
...
-spec subscribe(binary(), fun((lustre@internals@patch:patch(MRD)) -> nil)) -> lustre@internals@runtime:action(MRD, lustre:server_component()).
...
-spec do_select(
    fun((fun((MRR) -> nil), gleam@erlang@process:subject(any())) -> gleam@erlang@process:selector(MRR))
) -> lustre@effect:effect(MRR).
do_select(Sel) ->
...
-spec select(
    fun((fun((MRM) -> nil), gleam@erlang@process:subject(any())) -> gleam@erlang@process:selector(MRM))
) -> lustre@effect:effect(MRM).
select(Sel) ->
    do_select(Sel).
...
?DOC("\n").
-spec set_selector(
    gleam@erlang@process:selector(lustre@internals@runtime:action(any(), MRX))
) -> lustre@effect:effect(MRX).
set_selector(_) ->
    lustre@effect:from(
        fun(_) ->
            _pipe = <<"
